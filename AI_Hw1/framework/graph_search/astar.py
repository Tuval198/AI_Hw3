...
class AStar(BestFirstSearch):
    ...
    def _open_successor_node(self, problem: GraphProblem, successor_node: SearchNode):
        ...
        """
        node_in_close = self.close.get_node_by_state(successor_node.state)
        node_in_open = self.open.get_node_by_state(successor_node.state)

        if node_in_open:
            if successor_node.g_cost < node_in_open.g_cost:
                self.open.extract_node(node_in_open)
                self.open.push_node(successor_node)
        elif node_in_close:
            if successor_node.g_cost < node_in_close.g_cost:
                # add node of the same state with better score to open and remove from close:
